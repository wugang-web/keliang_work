...
class ConstantOutput:
    def __init__(self, window):
        ...
        self.label_param = tk.Label(window, text="常数值0：")
        self.label_param.place(x=900, y=230)

        self.entry1 = tk.Entry(window)
        self.entry1.insert(0, "1.0")  # 设置默认值为1.0
        self.entry1.place(x=900, y=260)
        self.entry1.bind("<KeyRelease>", self.output_param)  # 绑定键盘释放事件

        self.label_param_1 = tk.Label(window, text="常数值1：")
        self.label_param_1.place(x=900, y=330)

        self.entry2 = tk.Entry(window)
        self.entry2.insert(0, "1.0")  # 设置默认值为1.0
        self.entry2.place(x=900, y=360)
        self.entry2.bind("<KeyRelease>", self.output_param_1)  # 绑定键盘释放事件
        self.label_output = tk.Label(window, text="输出结果：")
        self.label_output.place(x=900, y=900)

        param = self.entry1.get()
        self.label_output.configure(text=f"输出结果：{param}")
        self.label_output.place(x=900, y=300)
        ...
        self.label_output_1.place(x=900, y=390)
...
def start_background_thread():
    ...
    background_thread = threading.Thread(target=read_data_background, args=(
        current_folder + r'\Real_time_read_file_data1.py', combo_combobox_chose.get(),))
...
def reset():
    ...
    # 增益器
    combo_combobox_1.current(0)
    gui.combo_combobox_2.current(0)
    # 加法器
